...
function resolveDeps(ast, result) {
  ...
  ast.walkRules(moduleDeclaration, rule => {
    if (importDeclaration.exec(rule.selector)) {
      ...
      )) pendingModules.push(absDependencyPath);
      else updateTranslations(translations, graph[absDependencyPath].exports);
  ...
  const lazyResult = pendingModules.length === 0
    ? Promise.resolve()
    : Promise.all(pendingModules.map(readFile))
      .then((...modules) => modules.reduce((chain, css, position) => {
        const absDependencyPath = pendingModules[position];

        return chain
          .then(() => processor.process(css, Object.assign({}, result.opts, {from: absDependencyPath})))
          .then(result => updateTranslations(translations, result.root.exports));
      }, Promise.resolve()));

  return lazyResult.then(() => {
